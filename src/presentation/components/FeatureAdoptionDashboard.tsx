...
export function FeatureAdoptionDashboard() {
  ...
  return (
    <div className="space-y-6 font-sans animate-fade-in">
      ...
      <div className="p-6 rounded-2xl bg-card border border-purple-500/30 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-md text-[10px] font-bold bg-purple-500/10 text-purple-500 border border-purple-500/20 uppercase tracking-wider">
      ...
      {isLoading ? (
        <div className="py-24 text-center text-muted-foreground space-y-3">
          ...
          <p className="text-xs font-semibold text-foreground">Carregando métricas de uso de IA e retenção de recursos...</p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div className="p-4 rounded-2xl bg-card border border-border space-y-1 shadow-xs">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Total de Chamadas IA</span>
              <strong className="text-3xl font-black text-foreground">{adoptionData?.totalAiExecutions}</strong>
              <span className="text-[10px] text-muted-foreground block">execuções computadas</span>
            </div>

            <div className="p-4 rounded-2xl bg-card border border-border space-y-1 shadow-xs">
              <span className="text-[10px] font-bold text-purple-500 uppercase tracking-wider block">Latência Média da IA</span>
              <strong className="text-3xl font-black text-purple-500">{adoptionData?.avgPlatformLatencySeconds}s</strong>
              <span className="text-[10px] text-muted-foreground block">tempo de resposta médio</span>
            </div>

            <div className="p-4 rounded-2xl bg-card border border-border space-y-1 shadow-xs">
              <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider block">Recursos em Alta Adoção</span>
              <strong className="text-3xl font-black text-emerald-500">{mostUsed.length}</strong>
              <span className="text-[10px] text-muted-foreground block">ferramentas com uso consistente</span>
            </div>

            <div className="p-4 rounded-2xl bg-card border border-red-500/20 bg-red-500/5 space-y-1 shadow-xs">
              <span className="text-[10px] font-bold text-red-500 uppercase tracking-wider block">Nunca Utilizadas (0 Uso)</span>
              <strong className="text-3xl font-black text-red-500">{neverUsed.length}</strong>
              <span className="text-[10px] text-red-500/80 block">zero execuções no período</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-5 rounded-2xl bg-card border border-border space-y-3 shadow-xs">
              ...
              <div className="space-y-2">
                {mostUsed.map((feat, idx) => (
                  <div key={feat.id} className="p-3 rounded-xl bg-card/80 border border-border flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <span className="w-6 h-6 rounded-lg bg-emerald-500/20 text-emerald-500 font-bold text-xs flex items-center justify-center border border-emerald-500/30 shrink-0">
                        #{idx + 1}
                      </span>
                      <div>
                        <strong className="text-xs font-bold text-foreground block">{feat.name}</strong>
                        <span className="text-[10px] text-muted-foreground">{feat.category} • {feat.activeUsersCount} usuários</span>
                      </div>
                    </div>

                    <div className="text-right shrink-0">
                      <strong className="text-sm font-black text-foreground block">{feat.totalUsage}</strong>
            ...
            <div className="p-5 rounded-2xl bg-card border border-amber-500/20 bg-amber-500/5 space-y-3 shadow-xs">
              <div className="flex items-center justify-between border-b border-border pb-2.5">
                ...
                <span className="text-[9px] text-amber-500 font-mono font-bold bg-amber-500/20 px-2 py-0.5 rounded border border-amber-500/30">
              ...
              <div className="space-y-2">
                {lowAdoption.length === 0 ? (
                  ...
                  lowAdoption.map((feat) => (
                    <div key={feat.id} className="p-3 rounded-xl bg-card/80 border border-amber-500/20 flex items-center justify-between gap-3">
                      <div>
                        <strong className="text-xs font-bold text-foreground block">{feat.name}</strong>
                        <span className="text-[10px] text-amber-500">{feat.category} • Uso abaixo de 5% do volume líder</span>
                      </div>

                      <div className="text-right shrink-0">
                        <strong className="text-sm font-black text-amber-500 block">{feat.totalUsage} usos</strong>
                        <span className="text-[10px] text-muted-foreground">{feat.frequencyPerUser}x / usuário</span>
                      </div>
                    </div>
                  ))
                )}
            ...
            <div className="p-5 rounded-2xl bg-card border border-red-500/20 bg-red-500/5 space-y-3 shadow-xs">
              <div className="flex items-center justify-between border-b border-border pb-2.5">
                ...
                <span className="text-[9px] text-red-500 font-mono font-bold bg-red-500/20 px-2 py-0.5 rounded border border-red-500/30">
                  Zero Chamadas
                </span>
              </div>
              ...
              <div className="space-y-2">
                {neverUsed.length === 0 ? (
                  ...
                  neverUsed.map((feat) => (
                    <div key={feat.id} className="p-3 rounded-xl bg-card/80 border border-red-500/20 flex items-center justify-between gap-3">
                      <div>
                        <strong className="text-xs font-bold text-foreground block">{feat.name}</strong>
                        <span className="text-[10px] text-red-500">{feat.category} • Nenhuma execução no período</span>
                      </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-border text-xs">
                  {allFeatures.map((feat: FeatureAdoptionItem) => (
                    <tr key={feat.id} className="hover:bg-card/80 transition">
                      <td className="py-3 px-3">
                        <strong className="text-foreground block font-bold">{feat.name}</strong>
                      ...
                      <td className="py-3 px-3 text-center">
                        <div className="inline-flex flex-col items-center">
                          <span className="font-bold text-brand-500">{feat.conversionToApplicationRate}%</span>
                          <span className="text-[9px] text-muted-foreground">(Fin. Pendente)</span>
                        </div>
                      </td>

                      <td className="py-3 px-3 text-center">
                        <span className="px-2 py-1 rounded text-[9px] font-bold bg-purple-500/10 text-purple-500 border border-purple-500/20 font-mono inline-block">
